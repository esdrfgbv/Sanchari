...
const visualizerCodeEl = document.getElementById("visualizerCode");
const visualizerTitleEl = document.getElementById("visualizerTitle");
const visualizerMetaEl = document.getElementById("visualizerMeta");
...
function showChallengePage(pushHistory = true) {
  if (!isMainAppPage) return;
  if (!challengeSection || !visualizerPage) return;
...
function showVisualizerPage(pushHistory = true) {
  if (!isMainAppPage) return;
  if (!challengeSection || !visualizerPage) return;
...
// All state lives inside this closure so we don't pollute globals
const visualizerEngine = (function () {
  // DOM references (re-use the ones we created at the top)
  const codeEl = visualizerCodeEl;
  const titleEl = visualizerTitleEl;
  const metaEl = visualizerMetaEl;
...
function pushSnapshot() {
  ...
  function ensureOutputBox() {
    ...
    const logSection = execLogEl.parentElement;
...
if (allowInput && !contextLabel) {
  ...
  // Entry point: load from a payload coming from challenge page
...
// Back to challenge inside embedded visualizer
if (backToChallengeBtn && isMainAppPage) {
  backToChallengeBtn.addEventListener("click", () => {
    // Just switch back to the same challenge section
    showChallengePage();
  });
}


// Visualize Code button → use embedded visualizer page
if (visualizeCodeBtn && isMainAppPage) {
  visualizeCodeBtn.addEventListener("click", () => {
    ...
    // Optional: keep in localStorage (for safety / reload)
    localStorage.setItem(
      "sanchari_visualizer_payload",
      JSON.stringify(payload)
    );

    // Load into engine + show visualizer SPA section
    visualizerEngine.loadFromPayload(payload);
    showVisualizerPage();
  });
}
...
if (backToHome) {
  backToHome.addEventListener("click", () => {
    // Go directly to home using the SPA navigation
    showHome(true);  // pushHistory = true so history.state is also correct
  });
}

if (backToLevels) {
  backToLevels.addEventListener("click", () => {
    // This one can safely use history since it's always from challenge → levels
    history.back();
  });
}
